...

{
  options.homelab.services.backup = {
    ...
    local.targetDir = lib.mkOption {
      ...
      default = "${homelab.mounts.merged}/Backups/Restic";
      type = lib.types.path;
    };
  };
  config =
    ...
    lib.mkIf (cfg.enable && enabledServices != { }) {
      systemd.tmpfiles.rules = lib.lists.optionals cfg.local.enable [
        "d ${cfg.local.targetDir} 0770 ${homelab.user} ${homelab.group} - -"
      ];
      ...
      systemd.services.restic-rest-server.serviceConfig = lib.attrsets.optionalAttrs cfg.local.enable {
        User = lib.mkForce homelab.user;
        Group = lib.mkForce homelab.group;
      };
      ...
      services.restic = {
        ...
        backups =
          ...
          // lib.attrsets.optionalAttrs (cfg.s3.enable && homelab.services.paperless.enable) {
            paperless-s3 =
              let
              ...
              in
              {
                ...
                paths = [
                  homelab.services.paperless.mediaDir
                ];
                backupPrepareCommand =
                  let
